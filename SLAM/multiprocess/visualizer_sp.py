...
class SLAM_GUI(object):
    def __init__(self, slam, args, params_gui=None, dataset=None, tracker2gui_call, tracker2gui_frame_queue):
        Log("GUI Constructor")
        self.step = 0
    ...
    def receive_data(self):
        # with self._tracker2gui_call:
        #     if self._tracker2gui_frame_queue.qsize() == 0:
        #         Log("waiting tracker to wakeup")
        #         self._tracker2gui_call.wait()

        if self._tracker2gui_frame_queue is None:
            return

        self.gui_packet = get_latest_queue(self._tracker2gui_frame_queue)
    ...
    def rasterise(self, current_cam):
        ...
        rendering_data = self.gui_renderer.render(current_cam, global_params)
    ...
    def render_gui(self):
        if not self.init or self.gui_packet is None:
            return
